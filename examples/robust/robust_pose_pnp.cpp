...
PoseEstimator::PoseEstimator(
    const std::vector<sqpnp::_Point>      *_3dpoints,
    const std::vector<sqpnp::_Projection> *_projections, int samplesz,
    int nm_samplesz) {
...
// Inlier and outlier indices are optionally returned (in increasing order) in
// idxInliers & idxOutliers. Returns 0 if successful, zero otherwise
int PoseEstimator::ransacfit(int miniter, int maxiter, double inlPcent,
                             double                      outlThresh,
                             robust_pose_pnp::Matrix34d &best_pose,
                             std::vector<int>           *idxInliers,
                             std::vector<int>           *idxOutliers) const {
  ransac_lib::LORansacOptions options;

  options.min_num_iterations_       = (uint32_t) miniter;
  options.max_num_iterations_       = (uint32_t) maxiter;
  // predicted #iterations for the given outlier ratio and sample size:
  // ransac_lib::utils::NumRequiredIterations(inlPcent, 1.0-0.99, samplesz_,
  ...
  ransac_lib::LocallyOptimizedMSAC<
      robust_pose_pnp::Matrix34d,
      std::vector<robust_pose_pnp::Matrix34d,
                  Eigen::aligned_allocator<robust_pose_pnp::Matrix34d>>,
  ...
  int num_inliers =
      lomsac.EstimateModel(options, *this, &best_pose, &ransac_stats);
...
int PoseEstimator::sqpnp_solve(
    const std::vector<int>                                            &sample,
    std::vector<robust_pose_pnp::Matrix34d,
                Eigen::aligned_allocator<robust_pose_pnp::Matrix34d>> *poses)
    const {
  ...
  if (solver.IsValid()) {
    ...
    for (int i = 0; i < solver.NumberOfSolutions(); i++) {
      ...
      // std::cout << " Average squared projection error : " <<
      // solver.AverageSquaredProjectionErrors().at(i) << std::endl;

      (*poses)[i].block<3, 3>(0, 0) =
          Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(
              sol->r_hat.data());
      (*poses)[i].block<3, 1>(0, 3) =
          Eigen::Map<const Eigen::Matrix<double, 3, 1, Eigen::ColMajor>>(
              sol->t.data());
    }
  } else {
    return 0;
...
#else
// this uses directly the correspondences using the weights: 1 for those in
// sample, 0 for the rest
int PoseEstimator::sqpnp_solve(
    const std::vector<int>                                            &sample,
    std::vector<robust_pose_pnp::Matrix34d,
                Eigen::aligned_allocator<robust_pose_pnp::Matrix34d>> *poses)
  ...
  for (int i = 0; i < nsample; ++i) { wghts[sample[i]] = 1.0; }
...
int PoseEstimator::MinimalSolver(
    const std::vector<int>                                            &sample,
    std::vector<robust_pose_pnp::Matrix34d,
                Eigen::aligned_allocator<robust_pose_pnp::Matrix34d>> *poses)
    const {
...
int PoseEstimator::NonMinimalSolver(const std::vector<int>     &sample,
  ...
  // note that RansacLib might call the non minimal solver with fewer than
  // nm_samplesz_ points, hence the following check
  ...
  if (n > 1) {
    std::cerr
        << "More than one solution in PoseEstimator::NonMinimalSolver()!\n"
        << std::flush;
  }
...
double
PoseEstimator::EvaluateModelOnPoint(const robust_pose_pnp::Matrix34d &pose,
                                    int                               i) const {
...
#else
  Eigen::Vector3d prod =
      pose.block<3, 3>(0, 0) * xyz.vector + pose.block<3, 1>(0, 3);
  double Xc = prod(0), Yc = prod(1), inv_Zc = 1. / prod(2);
