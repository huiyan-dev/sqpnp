...
void GenerateSyntheticPoints(int                               n,
  ...
  assert(n > 2);
  // intrinsics
  cv::Matx<double, 3, 3> K(1400, 0, 1000, 0, 1400, 900, 0, 0, 1);
  // 2d pixel noise, fx and fy are equal, if not should uese std_noise_x,
  // std_noise_y
  ...
  // The generator is Generated Point = std::normal_distribution<double>(C,
  // radius), radius)
  ...
  double s = (1 - sq_norm_psi) * inv_w, v1 = 2 * psi[0] * inv_w,
         v2 = 2 * psi[1] * inv_w, v3 = 2 * psi[2] * inv_w;
  ...
  cv::Vec<double, 3>               pos(camera_position(generator),
                         camera_position(generator),
                         camera_position(generator));
  ...
  while (points.size() < n) {
    ...
    cv::Vec<double, 3> Mw(point_X(generator),
                          point_Y(generator),
                          point_Z(generator));
...
// compute the translational and angular error between the pose R,t contained in
// an SQPnP solution and the (true) pose Rg,tg
static void poseError(const sqpnp::SQPSolution&     solution,
                      const cv::Matx<double, 3, 3>& Rg,
                      const cv::Vec<double, 3>& tg, double& terr,
                      double& aerr) {
  // translational error
  ...
  /* angular error, defined as the amount of rotation about a unit vector that
   * transfers Rg to R. The (residual) angle is computed with the inverse
   * Rodrigues rotation formula
   */

  // compute trc as the trace of Rg'*R
  a = Rg(0, 0) * solution.r_hat[0] + Rg(1, 0) * solution.r_hat[3] +
      Rg(2, 0) * solution.r_hat[6];
  b = Rg(0, 1) * solution.r_hat[1] + Rg(1, 1) * solution.r_hat[4] +
      Rg(2, 1) * solution.r_hat[7];
  c = Rg(0, 2) * solution.r_hat[2] + Rg(1, 2) * solution.r_hat[5] +
      Rg(2, 2) * solution.r_hat[8];
...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    ...
    std::cout << i << " translational error : " << terr
              << "  angular error : " << aerr * 180.0 / 3.14159 << "\n\n";
  }

  auto diff = finish - start;
  std::cout
      << " Average execution time : "
      << std::chrono::duration_cast<std::chrono::microseconds>(diff).count() / N
      << std::endl;
  std::cout << " Maximum squared error : " << max_sq_error << std::endl;
  std::cout << " Maximum average squared projection error : "
            << max_sq_proj_error << std::endl;
