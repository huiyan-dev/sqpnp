...
const OmegaNullspaceMethod SolverParameters::DEFAULT_OMEGA_NULLSPACE_METHOD =
    OmegaNullspaceMethod::RRQR;
const NearestRotationMethod SolverParameters::DEFAULT_NEAREST_ROTATION_METHOD =
    NearestRotationMethod::FOAM;
const double SolverParameters::DEFAULT_ORTHOGONALITY_SQUARED_ERROR_THRESHOLD =
    1e-8;
...
void PnPSolver::HandleSolution(SQPSolution& solution, double& min_sq_error) {
  bool cheirok =
      TestPositiveDepth(solution) ||
      TestPositiveMajorityDepths(
          solution);    // check the majority if the check with centroid fails
  if (cheirok) {
    solution.sq_error = (Omega_ * solution.r_hat).dot(solution.r_hat);
    if (fabs(min_sq_error - solution.sq_error) >
        parameters_.equal_squared_errors_diff) {
    ...
    } else    // look for a solution that's almost equal to this
    {
      ...
      for (int i = 0; i < num_solutions_; i++) {
        if ((solutions_[i].r_hat - solution.r_hat).squaredNorm() <
            parameters_.equal_vectors_squared_diff) {
      ...
      if (!found) { solutions_[num_solutions_++] = solution; }
      if (min_sq_error > solution.sq_error) {
        min_sq_error = solution.sq_error;
      }
    }
...
bool PnPSolver::Solve() {
  ...
  for (int i = 9 - num_eigen_points; i < 9; i++) {
    ...
    const Eigen::Matrix<double, 9, 1> e =
        SQRT3 *
        Eigen::Map<Eigen::Matrix<double, 9, 1>>(U_.block<9, 1>(0, i).data());
    ...
    if (orthogonality_sq_error <
        parameters_.orthogonality_squared_error_threshold) {
  ...
  int index, c = 1;
  while ((index = 9 - num_eigen_points - c) > 0 &&
         min_sq_error > 3 * s_[index]) {
    const Eigen::Matrix<double, 9, 1> e =
        Eigen::Map<Eigen::Matrix<double, 9, 1>>(
            U_.block<9, 1>(0, index).data());
    SQPSolution solution[2];
...
SQPSolution PnPSolver::RunSQP(const Eigen::Matrix<double, 9, 1>& r0) {
  ...
  while (delta_squared_norm > parameters_.sqp_squared_tolerance &&
         step++ < parameters_.sqp_max_iteration) {
...
void PnPSolver::SolveSQPSystem(const Eigen::Matrix<double, 9, 1>& r,
                               Eigen::Matrix<double, 9, 1>&       delta) {
  ...
  // and assign straight into delta
  delta = H * x;
  ...
  InvertSymmetric3x3(
      W,
      Winv);    // NOTE: This maybe also analytical with Eigen, but hey...
...
void PnPSolver::RowAndNullSpace(
    ...
    const double& norm_threshold       // Used to discard columns of Pn when
                                       // finding null space
    )    // threshold for column vector norm (of Pn)
{
  ...
  const Eigen::Matrix<double, 9, 9> Pn =
      Eigen::Matrix<double, 9, 9>::Identity() - (H * H.transpose());
