...
//     a) Paper:
//     https://www.ecva.net/papers/eccv_2020/papers_ECCV/papers/123460460.pdf b)
//     Supplementary:
//     https://www.ecva.net/papers/eccv_2020/papers_ECCV/papers/123460460-supp.pdf
...
public:
  ...
  const Eigen::Matrix<double, 9, 9>& Omega() const {
    return Omega_;
  }

  const Eigen::Matrix<double, 9, 9>& EigenVectors() const {
    return U_;
  }

  const Eigen::Matrix<double, 9, 1>& EigenValues() const {
    return s_;
  }

  const int NullSpaceDimension() const {
    return num_null_vectors_;
  }

  const int NumberOfSolutions() const {
  ...
  //
  // Constructor (initializes Omega, P and U, s, i.e. the decomposition of
  // Omega)
  ...
  inline PnPSolver(const std::vector<Point3D>& _3dpoints,
    ...
    num_null_vectors_ = -1;    // set to -1 in case we never make it to the
                               // decomposition of Omega
    Omega_            = Eigen::Matrix<double, 9, 9>::Zero();
    double sum_wx = 0.0, sum_wy = 0.0, sum_wx2_plus_wy2 = 0.0, sum_w = 0.0;

    double sum_wX = 0.0, sum_wY = 0.0, sum_wZ = 0.0;

    Eigen::Matrix<double, 3, 9> QA =
        Eigen::Matrix<double, 3, 9>::Zero();    // Sum( Qi*Ai )
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      const double wx = proj_[0] * w, wy = proj_[1] * w,
      ...
      double       X = pt_[0], Y = pt_[1], Z = pt_[2];
      ...
      // block (3:5, 3:5) because it's same as (0:2, 0:2)
      ...
      // Note that certain pairs of elements are equal, so we save some
      // operations by filling them outside the loop
      QA(0, 0) += wX;
      QA(0, 1) += wY;
      QA(0, 2) += wZ;
      QA(0, 6) += -wx * X;
      ...
      // QA(2, 0) += -wx*X; QA(2, 1) += -wx*Y; QA(2, 2) += -wx*Z; 	QA(2, 3) +=
      // -wy*X; QA(2, 4) += -wy*Y; QA(2, 5) += -wy*Z;
      QA(2, 6) += wsq_norm_m * X;
      QA(2, 7) += wsq_norm_m * Y;
      QA(2, 8) += wsq_norm_m * Z;
    }
    ...
    // Fill lower triangle of Omega; elements (7, 6), (8, 6) & (8, 7) have
    ...
    // Sum(A'*Qi*A') + Sum(Qi*Ai)'*inv(Sum(Qi))*Sum( Qi*Ai)
    ...
    if (parameters_.omega_nullspace_method == OmegaNullspaceMethod::RRQR) {
      // Rank revealing QR nullspace computation. This is slightly less accurate
      // compared to SVD but x2 faster
      Eigen::FullPivHouseholderQR<Eigen::Matrix<double, 9, 9>> rrqr(Omega_);
      U_ = rrqr.matrixQ();

      Eigen::Matrix<double, 9, 9> R =
          rrqr.matrixQR().template triangularView<Eigen::Upper>();
      s_ = R.diagonal().array().abs();
    } else    // if ( parameters_.omega_nullspace_method ==
              // OmegaNullspaceMethod::SVD )
    {
      // SVD-based nullspace computation. This is the most accurate but slowest
      // option
      Eigen::JacobiSVD<Eigen::Matrix<double, 9, 9>> svd(Omega_,
                                                        Eigen::ComputeFullU);
      U_ = svd.matrixU();
      s_ = svd.singularValues();
    }

    // Find dimension of null space; the check guards against overly large
    // rank_tolerance
    while (7 - num_null_vectors_ >= 0 &&
           s_[7 - num_null_vectors_] < _parameters.rank_tolerance) {
      num_null_vectors_++;
    }
    ...
    if (++num_null_vectors_ > 6) { flag_valid_ = false; }
    ...
    } else    // if ( parameters_.nearest_rotation_method ==
...
private:
  ...
  std::function<void(const Eigen::Matrix<double, 9, 1>&,
  ...
  void SolveSQPSystem(const Eigen::Matrix<double, 9, 1>& r,
  ...
  inline double AverageSquaredProjectionError(const int index) const {
    ...
    for (size_t i = 0; i < points_.size(); i++) {
      ...
      const double Xc = r[0] * M[0] + r[1] * M[1] + r[2] * M[2] + t[0],
                   ...
                   inv_Zc =
                       1.0 / (r[6] * M[0] + r[7] * M[1] + r[8] * M[2] + t[2]);
  ...
  // Simple SVD - based nearest rotation matrix. Argument should be a
  // *row-major* matrix representation. Returns a row-major vector
  // representation of the nearest rotation matrix.
  inline static void
  NearestRotationMatrix_SVD(const Eigen::Matrix<double, 9, 1>& e,
    ...
    Eigen::JacobiSVD<Eigen::Matrix<double, 3, 3>>       svd(E,
                                                      Eigen::ComputeFullU |
                                                          Eigen::ComputeFullV);
    double                                              detUV =
        Determinant3x3(svd.matrixU()) * Determinant3x3(svd.matrixV());
    // so we return back a row-major vector representation of the orthogonal
    // matrix
    Eigen::Matrix<double, 3, 3> R =
        svd.matrixU() *
        Eigen::Matrix<double, 3, 1>({1, 1, detUV}).asDiagonal() *
        svd.matrixV().transpose();
    r = Eigen::Map<Eigen::Matrix<double, 9, 1>>(R.data(), 9, 1);
  }

  // Faster nearest rotation computation based on FOAM. See M. Lourakis: "An
  // Efficient Solution to Absolute Orientation", ICPR 2016 and M. Lourakis, G.
  // Terzakis: "Efficient Absolute Orientation Revisited", IROS 2018.
   ...
   * Algorithm", J. Astronaut. Sci. 1993.
   *
   *  Copyright (C) 2019 Manolis Lourakis (lourakis **at** ics forth gr)
   *  Institute of Computer Science, Foundation for Research & Technology -
   * Hellas Heraklion, Crete, Greece.
   */
  inline static void
  NearestRotationMatrix_FOAM(const Eigen::Matrix<double, 9, 1>& e,
                             Eigen::Matrix<double, 9, 1>&       r) {
    int           i;
    ...
    for (i = 15, lprev = 0.0; fabs(l - lprev) > 1E-12 * fabs(lprev) && i > 0;
         --i) {
      double tmp, p, pp;
  ...
  OrthogonalityError(const Eigen::Matrix<double, 9, 1>& a) {
