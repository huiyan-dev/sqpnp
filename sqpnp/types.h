...
struct _Projection {
  Eigen::Matrix<double, 2, 1>
      vector;    // for older compilers, this might need to be declared as
                 // Eigen::Matrix<double, 2, 1, Eigen::DontAlign>

  inline _Projection() : vector(Eigen::Matrix<double, 2, 1>(0, 0)) {}

  template <typename P>
  inline _Projection(const P& _x, const P& _y) :
      vector(Eigen::Matrix<double, 2, 1>(_x, _y)) {}

  template <class Proj> inline _Projection(const Proj& _projection) {
    *this = _projection;
  }
...
struct _Point {
  ...
  template <typename P>
  inline _Point(const P& _x, const P& _y, const P& _z) :
      vector(Eigen::Matrix<double, 3, 1>(_x, _y, _z)) {}

  template <class Pt> inline _Point(const Pt& _point) {
    *this = _point;
  }

#ifdef HAVE_OPENCV
  template <typename P> _Point& operator=(const cv::Point3_<P>& _point) {
    vector[0] = _point.x;
    vector[1] = _point.y;
    vector[2] = _point.z;
    return *this;
  ...
  template <typename P> _Point& operator=(const cv::Vec<P, 3>& _point) {
    vector[0] = _point[0];
    vector[1] = _point[1];
    vector[2] = _point[2];
    return *this;
...
struct SolverParameters {
  ...
  inline SolverParameters(
      const double& _rank_tolerance        = DEFAULT_RANK_TOLERANCE,
      ...
      const OmegaNullspaceMethod& _omega_nullspace_method =
          DEFAULT_OMEGA_NULLSPACE_METHOD,
      const NearestRotationMethod& _nearest_rotation_method =
          DEFAULT_NEAREST_ROTATION_METHOD,
      const double& _orthogonality_squared_error_threshold =
          DEFAULT_ORTHOGONALITY_SQUARED_ERROR_THRESHOLD,
      const double& _equal_vectors_squared_diff =
          DEFAULT_EQUAL_VECTORS_SQUARED_DIFF,
      const double& _equal_squared_errors_diff =
          DEFAULT_EQUAL_SQUARED_ERRORS_DIFF,
      const double& _point_variance_threshold =
          DEFAULT_POINT_VARIANCE_THRESHOLD) :
      ...
      orthogonality_squared_error_threshold(
          _orthogonality_squared_error_threshold),
...
inline std::ostream& operator<<(std::ostream& os, const SQPSolution& solution) {
  return os << "\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
               "+++++++++++++++++++++++++++++++++++\n"
            << "R: [ " << solution.r_hat[0] << ", " << solution.r_hat[1] << ", "
            << solution.r_hat[2] << ";\n"
            << "     " << solution.r_hat[3] << ", " << solution.r_hat[4] << ", "
            << solution.r_hat[5] << ";\n"
            << "     " << solution.r_hat[6] << ", " << solution.r_hat[7] << ", "
            << solution.r_hat[8] << " ]\n"
            << "t: "
            << "[ " << solution.t[0] << "; " << solution.t[1] << "; "
            << solution.t[2] << " ]\n"
            << "r: "
            << "[ " << solution.r_hat[0] << "; " << solution.r_hat[1] << "; "
            << solution.r_hat[2] << "; " << solution.r_hat[3] << "; "
            << solution.r_hat[4] << "; " << solution.r_hat[5] << "; "
            << solution.r_hat[6] << "; " << solution.r_hat[7] << "; "
            << solution.r_hat[8] << " ]\n"
